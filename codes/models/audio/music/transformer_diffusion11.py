...
class ConcatAttentionBlock(TimestepBlock):
    def __init__(self, trunk_dim, heads, dropout):
        ...
        self.block3 = SubBlock(trunk_dim+contraction_dim*4, contraction_dim, heads, dropout)
        self.out = nn.Linear(contraction_dim*6, trunk_dim, bias=False)
...
class TransformerDiffusion(nn.Module):
    ...
    def __init__(
        ...
        self.layers = TimestepRotaryEmbedSequential(*[ConcatAttentionBlock(model_channels, num_heads, dropout) for _ in range(num_layers)])
...
class TransformerDiffusionWithQuantizer(nn.Module):
    ...
    def get_grad_norm_parameter_groups(self):
        ...
        attn3 = list(itertools.chain.from_iterable([lyr.block3.attn.parameters() for lyr in self.diff.layers]))
        ...
        ff3 = list(itertools.chain.from_iterable([lyr.block3.ff.parameters() for lyr in self.diff.layers]))
...
@register_model
def register_transformer_diffusion11(opt_net, opt):
    return TransformerDiffusion(**opt_net['kwargs'])


@register_model
def register_transformer_diffusion11_with_quantizer(opt_net, opt):
    return TransformerDiffusionWithQuantizer(**opt_net['kwargs'])


@register_model
def register_transformer_diffusion11_with_ar_prior(opt_net, opt):
