...
class SpecialSequential(nn.Sequential, TimestepBlock):
    def forward(self, x, aligned_emb, point_emb, rotary_emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, aligned_emb, point_emb, rotary_emb)
            else:
                x = layer(x, aligned_emb, rotary_emb)
        return x
...
class AttentionBlock(TimestepBlock):
    ...
    def forward(self, x, aligned_emb, timestep_emb, rotary_emb):
        h = self.intg(torch.cat([x, aligned_emb], dim=-1))
