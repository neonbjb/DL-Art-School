...
class DiffusionTtsFlat(nn.Module):
    def __init__(
        ...
        # transformer network.
        self.code_embedding = nn.ModuleList([nn.Embedding(8, model_channels//8) for _ in range(8)])
    ...
    def timestep_independent(self, aligned_conditioning, conditioning_input, expected_seq_len, return_code_pred):
        ...
        if is_latent(aligned_conditioning):
            ...
            code_emb = [embedding(aligned_conditioning[:, :, i]) for i, embedding in enumerate(self.code_embedding)]
...
@register_model
def register_flat_diffusion_retest(opt_net, opt):
